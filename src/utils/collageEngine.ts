...
export function calculateGrid(numImages: number, canvasWidth?: number, canvasHeight?: number): { rows: number; cols: number } {
  ...
  const aspectRatio = (canvasWidth && canvasHeight) ? canvasWidth / canvasHeight : 1;
  ...
  if (isSquare) {
    ...
    if (numImages === 3) return { rows: 2, cols: 2 };
    if (numImages === 4) return { rows: 2, cols: 2 };
    if (numImages === 5) return { rows: 2, cols: 3 };
    if (numImages === 6) return { rows: 2, cols: 3 };
    if (numImages === 7) return { rows: 3, cols: 3 };
    if (numImages === 8) return { rows: 3, cols: 3 };
    if (numImages === 9) return { rows: 3, cols: 3 };
    if (numImages === 10) return { rows: 3, cols: 4 };
    if (numImages <= 12) return { rows: 3, cols: 4 };
    if (numImages <= 16) return { rows: 4, cols: 4 };
    ...
    if (numImages <= 12) return { rows: 2, cols: 6 };
    if (numImages <= 16) return { rows: 2, cols: 8 };
  } else if (isTall) {
    if (numImages === 2) return { rows: 2, cols: 1 };
    if (numImages === 3) return { rows: 3, cols: 1 };
    if (numImages === 4) return { rows: 4, cols: 1 };
    if (numImages === 5) return { rows: 5, cols: 1 };
    if (numImages === 6) return { rows: 3, cols: 2 };
    if (numImages <= 8) return { rows: 4, cols: 2 };
    if (numImages <= 10) return { rows: 5, cols: 2 };
  }
  
  const cols = Math.ceil(Math.sqrt(numImages * aspectRatio));
